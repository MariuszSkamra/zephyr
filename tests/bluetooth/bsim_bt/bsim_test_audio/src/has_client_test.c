...
// TODO: Implement callback
static void has_preset_name_set_cb(struct bt_has *has, int err, char *name)
{
	if (err) {
		FAIL("HAS preset name changed cb err\n");
		return;
	}
	if (strlen(name) > sizeof(g_has_preset_name) - 1) {
		printk("Warning: HAS preset name (%zu) is larger than buffer (%zu)\n",
			strlen(name), sizeof(g_has_preset_name) - 1);
	}
	strncpy(g_has_preset_name, name, sizeof(g_has_preset_name) - 1);
	g_has_preset_name[sizeof(g_has_preset_name) - 1] = '\0';
	g_cb = true;
}

static struct bt_has_cb has_cb = {
	.active_preset_set = has_active_preset_set_cb,
	.active_preset_get = has_active_preset_get_cb,
	.active_preset_set_next = has_active_preset_set_next_cb,
	.active_preset_set_prev = has_active_preset_set_prev_cb,
	.preset_list_get = has_preset_list_get_cb,
	.preset_name_set = has_preset_name_set_cb,
};
